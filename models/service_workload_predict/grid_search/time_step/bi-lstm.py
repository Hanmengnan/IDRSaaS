...
class PyTorchGridSearchCV(BaseEstimator, RegressorMixin):
    def __init__(self, input_dim=4, output_dim=4, step_num=5, hidden_dim=16, num_layers=2, epochs=50, batch_size=16, device=None):
    ...
    def fit(self):
        self.model = BiLSTMModel(self.input_dim, self.output_dim, self.hidden_dim, self.num_layers).to(self.device)
        ...
        train(x_train, y_train, x_val, y_val, self.model, self.epochs, self.batch_size, self.device)
        return self
    ...
    def predict(self):
        ...
        predict_x = torch.tensor(x_test, dtype=torch.float32).to(self.device)
